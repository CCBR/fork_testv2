...
Analysis Genus OTU Count - By Control Type
```{r}
...
Analysis Genus OTU Count - By Kit
```{r}
...
write.csv(genus_otupercent_sample,"output.csv")

```

Analysis Genus Percent - By Control Type
```{r}
#Unique Types of controls
control_info$Type <- as.character(control_info$Type)
control_types <- unique(control_info$Type)
...
for (a in control_types){
  ...
  for (c in genus_list){
   ...
   for(b in sample_list){
    ...
    total <- genus_otupercent_sample [c,b] + total
   
   }
    percent <- total / length(sample_list)
    genus_otupercent_control[c,a] <- percent
  }
 }
