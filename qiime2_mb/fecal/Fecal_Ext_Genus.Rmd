...
```{r}
#Read in database with variable information and list of samples that did not meet the  threshold
...
#6. OTU Counts - Analysis Genus OTU Presence/Absence by Control Type by Kit (Ext Control Only)
...
write.csv(genus_otuabund_sample,"Data/genus_otuabund_sample.csv")
...
#Output to text file
write.csv(genus_otuabund_control,"Data/genus_otuabund_control.csv")
```

#9. OTU Percent - Analysis Genus OTU Abundance by Control Type by Kit (Ext Control Only)
```{r}
#Unique Types of controls and all genus
kit_types <- unique(control_info$Kit)
...
#Output to text file
write.csv(genus_percent,"Data/genus_outabund_kit.csv")
```

#10. OTU Percent - Analysis of Genus Percent Abundance Differences, expected versus observed by kit (Ext Only)
...
#Output file
write.csv(temp,"Data/genus_otuabund_obsVSexp_ext.csv")
```

#11. OTU Percent - Not expected Genus by Control Type by Kit (Ext Control Only)
```{r}
...
write.csv(temp,"Data/genus_other.csv")
```
...
genus_list <- rownames(art.col.con)
genus_list

## Zymo MagBead DNone Kit
#################
 temp<-data.frame()
 count=1
 
###Create database
 temp2 <- subset(control_info,Kit=="Zymo MagBead DNone Kit")
 ...
 for (a in genus_list){
   ...
   for(b in sample_list){
    temp[count,"Genus"]<-a
    temp[count,"value"]<- genus_otuabund_sample[a,b]
    temp[count,"Sample"]<-b
    temp[count,"Expected"]<-genus_percent[a,"Art.Col"]
    count=count+1
   }
 }
 temp
 ...
 #Plot Data
 ggplot(data=temp,mapping=aes(x=Expected,y=value,color=Sample))+
 geom_point()+ #Can add points to graph
 geom_smooth(method = "lm",se=FALSE)+ #Add a smooth line
 labs(title="Artifical Colony: Intrakit Variablity (Zymo MagBead DNone Kit)", x="Expected Percentage", y= "Actual Percentage")+
...
###Create database
 temp2 <- subset(control_info,Kit=="Qiagen DSP Virus")
 temp2 <- subset(temp2,Type=="AC")
 temp2
 ...
 geom_point()+ #Can add points to graph
 geom_smooth(method = "lm",se=FALSE)+ #Add a smooth line
 labs(title="Artifical Colony: Intrakit Variablity (Qiagen DSP)", x="Expected Percentage", y= "Actual Percentage")+
 geom_text(mapping=aes(x=Expected,y=value,label=Genus),check_overlap = TRUE)
 
#################
## MagAttract PowerMag Soil
#################
 temp<-data.frame()
 count=1
 
###Create database
 temp2 <- subset(control_info,Kit=="MagAttract PowerMag Soil")
 temp2 <- subset(temp2,Type=="AC")
 temp2
 ...
 geom_point()+ #Can add points to graph
 geom_smooth(method = "lm",se=FALSE)+ #Add a smooth line
 labs(title="Artifical Colony: Intrakit Variablity (Mag Attract PowerSoil)", x="Expected Percentage", y= "Actual Percentage")+
 geom_text(mapping=aes(x=Expected,y=value,label=Genus),check_overlap = TRUE)

#################
## MagAttract PowerMag Microbiome
#################
 temp<-data.frame()
 count=1
 
###Create database
 temp2 <- subset(control_info,Kit=="MagAttract PowerMag Microbiome")
 temp2 <- subset(temp2,Type=="AC")
 temp2
 ...
 geom_point()+ #Can add points to graph
 geom_smooth(method = "lm",se=FALSE)+ #Add a smooth line
 labs(title="Artifical Colony: Intrakit Variablity (Mag Attract Microbiome)", x="Expected Percentage", y= "Actual Percentage")+
 geom_text(mapping=aes(x=Expected,y=value,label=Genus),check_overlap = TRUE)
#################
## Qiagen QIAamp  Modified
#################
 temp<-data.frame()
 count=1
 
###Create database
 temp2 <- subset(control_info,Kit=="Qiagen QIAamp  Modified")
 temp2 <- subset(temp2,Type=="AC")
 temp2
 ...
 geom_point()+ #Can add points to graph
 geom_smooth(method = "lm",se=FALSE)+ #Add a smooth line
 labs(title="Artifical Colony: Intrakit Variablity (Qiagen QIAamp Modified)", x="Expected Percentage", y= "Actual Percentage")+
 geom_text(mapping=aes(x=Expected,y=value,label=Genus),check_overlap = TRUE)
```

#12B Plot graph - Actual to Expected - Intrakit (Zymo Ext Blank)
For each kit, show variability, and map
```{r}
#Create genus list for zymo extraction control only
genus_list <- rownames(zymo.ext.con)
genus_list
...
###Create database
 ...
 for (a in genus_list){
   ...
   for(b in sample_list){
    ...
    temp[count,"value"]<- genus_otuabund_sample[a,b]
    temp[count,"Sample"]<-b
    temp[count,"Expected"]<-genus_percent[a,"Zymo.Ext"]
    count=count+1
   }
 }
 temp
 ...
 geom_point()+ #Can add points to graph
 geom_smooth(method = "lm",se=FALSE)+ #Add a smooth line
 labs(title="Zymo Ext Control: Intrakit Variablity (Zymo MagBead DNone Kit)", x="Expected Percentage", y= "Actual Percentage")+
 geom_text(mapping=aes(x=Expected,y=value,label=Genus),check_overlap = TRUE)
...
###Create database
 ...
 geom_point()+ #Can add points to graph
 geom_smooth(method = "lm",se=FALSE)+ #Add a smooth line
 labs(title="Zymo Ext Control: Intrakit Variablity (Qiagen DSP)", x="Expected Percentage", y= "Actual Percentage")+
 geom_text(mapping=aes(x=Expected,y=value,label=Genus),check_overlap = TRUE)
...
###Create database
 ...
 geom_point()+ #Can add points to graph
 geom_smooth(method = "lm",se=FALSE)+ #Add a smooth line
 labs(title="Zymo Ext Control: Intrakit Variablity (Mag Attract PowerSoil)", x="Expected Percentage", y= "Actual Percentage")+
 geom_text(mapping=aes(x=Expected,y=value,label=Genus),check_overlap = TRUE)

 #Remove outliers, and re-plot Data
 temp <- subset(temp,!(Sample=="SC249408.PC04925.E02") | !(Sample=="SC249408.PC07578.E02"))
 unique(temp$Sample)
...
###Create database
 ...
 geom_point()+ #Can add points to graph
 geom_smooth(method = "lm",se=FALSE)+ #Add a smooth line
 labs(title="Zymo Ext Control: Intrakit Variablity (Mag Attract Microbiome)", x="Expected Percentage", y= "Actual Percentage")+
 geom_text(mapping=aes(x=Expected,y=value,label=Genus),check_overlap = TRUE)
#################
## Qiagen QIAamp  Modified
#################
...
###Create database
 ...
 geom_point()+ #Can add points to graph
 geom_smooth(method = "lm",se=FALSE)+ #Add a smooth line
 labs(title="Zymo Ext Control: Intrakit Variablity (Qiagen QIAamp Modified)", x="Expected Percentage", y= "Actual Percentage")+
 geom_text(mapping=aes(x=Expected,y=value,label=Genus),check_overlap = TRUE)
```

#13. Plot graph - Actual to Expected - Between kits
```{r}
###############
#Zymo Ext
###############
...
zymo.kits <- genus_percent[,c("Zymo.Ext","Genus","Qiagen DSP Virus_OB_ZE","Zymo MagBead DNone Kit_OB_ZE","MagAttract PowerMag Soil_OB_ZE","Qiagen QIAamp  Modified_OB_ZE", "MagAttract PowerMag Microbiome_OB_ZE")]

zymo.kits <- subset(zymo.kits,!(is.na(Zymo.Ext)))
rownames(zymo.kits)
colnames(zymo.kits)
...
for (a in c(3:7)){
 ...
 for(b in 1:nrow(zymo.kits)){
  temp3[count,"Expected"]<- zymo.kits[count2,"Zymo.Ext"]
  temp3[count,"Genus"] <- rownames(zymo.kits[b,])
  temp3[count,"value"] <- zymo.kits[b,a]
  temp3[count,"Kit"] <- colnames(zymo.kits[a])
  count=count+1
  count2=count2+1
 }
}
...
#Plot graph
ggplot(data=temp3,mapping=aes(x=Expected,y=value,color=Kit))+
 #geom_point()+ #Can add points to graph
 geom_smooth(method = "lm",se=FALSE)+ #Add a smooth line
 labs(title="Zymo.Extraction-Nephele", x="Expected Percentage", y= "Actual Percentage")+
 geom_text(mapping=aes(x=Expected,y=value,label=Genus),check_overlap = TRUE)
...
#Create dataframe
colnames(genus_percent)
ac.kits <- genus_percent[,c("Art.Col","Genus","Qiagen DSP Virus_OB_AC","Zymo MagBead DNone Kit_OB_AC","MagAttract PowerMag Soil_OB_AC","Qiagen QIAamp  Modified_OB_AC", "MagAttract PowerMag Microbiome_OB_AC")]

ac.kits <- subset(ac.kits,!(is.na(Art.Col)))
rownames(ac.kits)
colnames(ac.kits)
...
for (a in c(3:7)){
 count2=1
 for(b in 1:nrow(ac.kits)){
  temp3[count,"Expected"]<- ac.kits[count2,"Art.Col"]
  temp3[count,"Genus"] <- rownames(ac.kits[b,])
  temp3[count,"value"] <- ac.kits[b,a]
  temp3[count,"Kit"] <- colnames(ac.kits[a])
  count=count+1
  count2=count2+1
 }
}
...
ggplot(data=temp3,mapping=aes(x=Expected,y=value,color=Kit))+
 #geom_point()+ #Can add points to graph
 geom_smooth(method = "lm",se=FALSE)+ #Add a smooth line
 labs(title="Artificial Colony-Nephele", x="Expected Percentage", y= "Actual Percentage")+
 geom_text(mapping=aes(x=Expected,y=value,label=Genus),check_overlap = TRUE)

```

#14. Unexpected by kit
```{r}
#Create temp dataframe from genus_percent
colnames(genus_percent)
temp <- genus_percent[,c(11,16,23:32)]
...
head(temp2)
write.csv(temp2,"Data/genus_otuabund_ext_neph_other_tab.csv")
```